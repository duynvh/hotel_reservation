...
class RoomTypeService
{
    ...
    public function __construct(RoomTypeRepositoryInterface $roomTypeRepository, ImageOption $imageOption)
    {
        $this->roomTypeRepository = $roomTypeRepository;
        $this->imageOption = $imageOption;
        $this->infoBasic = $this->roomTypeRepository->getInfoBasic();
    }

    public function index($params)
    {
        $data = $this->roomTypeRepository->getRoomType($params);
        $dataAll = $this->roomTypeRepository->getAll();
        $dataAllHidden = $this->roomTypeRepository->getAllRoomTypeConfirm();
        return [
            'data' => $data,
            'dataAll' => $dataAll,
    ...
    {
        $data = $this->roomTypeRepository->getRoomTypeNeedConfirm($params);
        $dataAll = $this->roomTypeRepository->getAll();
        return [
            'data' => $data,
            'dataAll' => $dataAll,
    ...
    {
        $item = $this->roomTypeRepository->getItemConfirm($id);
        if ($item->confirm_action === 'add') {
            ...
            $this->roomTypeRepository->destroyHidden($id);
            ...
            $this->roomTypeRepository->updateHidden($data, $id);
    ...
    public function store($request)
    {
        if ($request->image != null) {
            ...
            if ($move) {
                ...
                $this->roomTypeRepository->store($data);
...
//
    ...
    {
        $data = $this->roomTypeRepository->getItemConfirm($id);
    ...
    {
        $data = $this->roomTypeRepository->find($id);
        return [
            'infoBasic' => $this->infoBasic,
            'data' => $data,
        ];
    ...
    {
        $item = $this->roomTypeRepository->find($id);
        ...
        $this->roomTypeRepository->update($data, $id);
        return redirect()
            ->route($this->infoBasic['route'] . '.index')
            ->with(['noticeMessage' => Config::get('constants.SUCCESSFUL_MESSAGE.EDIT')]);
    }
    ...
    public function changeStatus($request)
    {
        ...
        if (!in_array($to_status, array('to-active', 'to-inactive')) || empty($ids)) {
            ...
            $this->roomTypeRepository->changeStatus($ids, $status);
    ...
    public function delete($request)
    {
        ...
        if (empty($ids)) {
            ...
            if (Gate::allows('editor', Auth::user())) {
                ...
                $this->roomTypeRepository->update($data, $ids);
            } else {
